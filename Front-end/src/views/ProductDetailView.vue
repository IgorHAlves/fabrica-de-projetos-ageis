...
async function loadProductData() {
  ...
  if (isLoadingData && currentProductId === productId && loadPromise) {
    if (import.meta.env.DEV) {
      console.log('loadProductData: Já está carregando o mesmo produto, retornando promise existente')
    }
    return loadPromise
  }
  ...
  if (isLoadingData && currentProductId !== productId) {
    if (import.meta.env.DEV) {
      console.log('loadProductData: Aguardando carregamento anterior terminar')
    }
    if (loadPromise) {
      await loadPromise
    }
  ...
  if (isLoadingData && currentProductId === productId) {
    if (import.meta.env.DEV) {
      console.log('loadProductData: Já está carregando, ignorando chamada duplicada')
    }
    return
  }
  ...
  loadPromise = (async () => {
    ...
    try {
      ...
      if (import.meta.env.DEV) {
        console.log('loadProductData: Carregando produto com ID:', productId)
      }
      ...
      if (import.meta.env.DEV) {
        console.log('loadProductData: Produto carregado:', product.value ? 'Sucesso' : 'null/undefined')
      }
    ...
    if (idPai) {
      try {
        ...
        if (import.meta.env.DEV) {
          console.log('Produto pai carregado:', parentProduct.value)
        }
      } catch (parentError) {
        if (import.meta.env.DEV) {
    ...
    try {
      ...
      if (import.meta.env.DEV) {
        console.log(`Encontradas ${variations.value.length} variações para o produto ${parentId}`)
      }
